      ...
      for (const tf of timeframes) {
        ...
        if (!pre.pass) {
          ...
          if (logSpamControlled(stopBurstKey, String(pre.reason||'').toLowerCase(), 1200) && logSpamControlled(preStopKey, `STOP|${String(pre.reason||'').toLowerCase()}`, 30000)) {
            ...
            if (['IMPULSE_EXTENSION', 'MICRO_NOISE', 'DIR_STABILIZER'].includes(reasonTxt)) {
              const bgReason = reasonTxt === 'IMPULSE_EXTENSION'
                ? 'ИМПУЛС'
                : reasonTxt === 'MICRO_NOISE'
                  ? 'ШУМ'
                  : 'СТАБИЛИЗАТОР';
        ...
        const confBin = Math.floor((Number(decision.confidence||0) * 100) / 5) * 5;
        ...
        if (logSpamControlled(passBurstKey, `${String(decision.direction||'').toUpperCase()}|${confBin}`, 1500) && logSpamControlled(prePassKey, `PASS|${String(decision.direction||'').toUpperCase()}`, 15000)) {
          logConsoleLine(`🟢🧭 ПРЕЧЕК УСПЕШЕН | ${String(tf||'1m').toLowerCase()} | ${decision.direction} | ${(Number(decision.confidence||0)*100).toFixed(0)}%`);
...
const readySignals = Object.keys(tfStatus)
      ...
      for (let i = 0; i < signalsToExecute.length; i += 1) {
        ...
        const requiredThresholdPre = clamp01(getKillerThresholdForScope(assetScopePre));
...
const signal = {
        ...
        const confirmDelayMs = Math.max(0, Math.min(1200, Number.isFinite(S.confirmDelayMs) ? S.confirmDelayMs : 200));
        ...
        if (confirmDelayMs > 0) {
          ...
          decision2 = selectBestStrategyDecision(strategyDecisions2);
        ...
        const finalConfPct = Math.round(Number(decision2?.confidence ?? decision?.confidence ?? 0) * 100);
